...
export const getFleetUtilization = async (req, res) => {
  try {
    ...
    const activeShips =
      fleetStats.find((stat) => stat.status === "active")?._count.id || 0;
    const maintenanceShips =
      fleetStats.find((stat) => stat.status === "under_maintenance")?._count
        .id || 0;
    const decommissionedShips =
      fleetStats.find((stat) => stat.status === "decommissioned")?._count.id ||
      0;

    const utilizationRate =
      totalShips > 0 ? Math.round((activeShips / totalShips) * 100) : 0;
  ...
  try {
      ...
      if (shipment.status === "delivered") {
        ...
        if (
          ...
          const delayDays = Math.max(
            0,
            (actual - estimated) / (1000 * 60 * 60 * 24)
          );
    ...
    const routeEfficiency = Array.from(routeMap.values())
      ...
      .map((route) => ({
        ...
        avgDelay:
          route.shipments > 0
            ? Number((route.totalDelay / route.shipments).toFixed(1))
            : 0,
        efficiency:
          route.shipments > 0
            ? Math.max(
                50,
                100 -
                  (route.delayed / route.shipments) * 100 -
                  (route.totalDelay / route.shipments) * 5
              )
            : 0,
        status:
          route.shipments > 0
            ? 100 - (route.delayed / route.shipments) * 100 >= 90
              ? "Excellent"
              : 100 - (route.delayed / route.shipments) * 100 >= 80
              ? "Good"
              : "Needs Attention"
            : "No Data",
      }))
    ...
    const portPerformance = portShipments
      .map((port) => {
        const totalShipments =
          port.shipmentsOrigin.length + port.shipmentsDestination.length;
        ...
        const efficiency = Math.max(
          70,
          95 - (totalShipments > 50 ? 10 : totalShipments > 30 ? 5 : 0)
        );
        ...
        return {
          ...
          rating:
            efficiency >= 90 ? "Excellent" : efficiency >= 80 ? "Good" : "Fair",
        };
      })
...
export const getDashboardAnalytics = async (req, res) => {
  try {
    ...
    const [fleetData, routeData, crewData, portData, insightData] =
      await Promise.all([
...
async function getRouteEfficiencyData() {
  ...
  return {
    ...
    avgEfficiency: Math.round(
      85 - (delayedShipments / Math.max(1, totalShipments)) * 20
    ),
    totalShipments,
  };
}
